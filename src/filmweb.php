...
function get_webpage_link_by_id(int $id)
{
    return 'https://www.filmweb.pl/film/a-1-' . $id;
}
...
function get_webpage_by_id(int $id)
{
    ...
    curl_setopt($ch, CURLOPT_URL, get_webpage_link_by_id($id));
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
    curl_setopt($ch, CURLOPT_ENCODING, 'gzip, deflate');
...
function get_movie_id_from_name(string $movieName)
{
    $database = get_db();
...
function get_poster(string $movieName)
{
    ...
    $id = get_movie_id_from_name($movieName);
    if ($id == null) {
